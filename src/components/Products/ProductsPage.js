...
const ProductsPage = ({ darkMode, toggleDarkMode, unreadCount, shouldAnimate})=> {
...
//   };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        try {
          ...
          const response = await fetchProducts(0, 12,  filters, searchQuery);
  ...
  const loadMorePosts = async () => {
    ...
    try {
      setLoadingMore(true);
      const response = await fetchProducts(skip, 12,  filters, searchQuery);
      const newPosts = response.data.posts || [];
